...
var tempMarkerIndex = null;
var tempInfoWindow = [];

function controllingZoomMaps(ooh_id) {
	var getMarker = oohmarker.findIndex((item) => parseInt(item.data_options.ooh_id) === parseInt(ooh_id));

	console.log("CLICK ZOOM", oohmarker[getMarker]);
	
	gmap.setZoom(25);
	gmap.panTo(oohmarker[getMarker].position);

	addInfoWindow(oohmarker[getMarker], getMarker, oohmarker[getMarker].data_options.name);
}

function addInfoWindow(marker, index, message) {
	tempInfoWindow[index] = new google.maps.InfoWindow({
		content: message
	});

	if(tempMarkerIndex !== null) tempInfoWindow[tempMarkerIndex].close();

	tempInfoWindow[index].open(gmap, marker);
